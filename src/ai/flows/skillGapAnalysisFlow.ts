...
const skillGapAnalysisPrompt = ai.definePrompt({
  ...
  prompt: `You are an expert career coach and skills analyst. Your task is to perform a detailed skill gap analysis by comparing a user's resume against a target job description.
...
Instructions for Skill Gap Analysis:
...
3.  **Find Matching Skills:** Compare the 'identifiedUserSkills' with 'identifiedJobRequirements'. List all skills that are clearly present in both. Populate the 'matchingSkills' array.
4.  **Identify Missing Skills:** Identify key skills/requirements from the job description that are NOT EVIDENT or NOT SUFFICIENTLY EMPHASIZED in the user's resume. Populate the 'missingSkills' array. Focus on actionable gaps.
5.  **Skill Gap Summary:** Provide a concise summary (2-3 sentences) of the overall alignment. Mention if the user is a strong, moderate, or weak fit based on the skill overlap, and briefly highlight the most critical gaps. Populate the 'skillGapSummary' string.
...
7.  **Overall Fit Score:** Based on your comprehensive analysis of matching and missing skills, provide an 'overallFitScore' (a number between 0 and 100) representing your estimation of how well the user's current resume aligns with the job requirements. A higher score indicates a better fit.
